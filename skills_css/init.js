...
function aali_tm_service_popup(){
	...
	button.on('click',function(){
		...
		modalBox.find('.service_popup_informations').prepend('<div class="image"><img src="https://shtheme.com/demosd/aali/wp-content/uploads/2022/02/4-2.jpg" alt="" /><div class="main" data-img-url="'+elImage+'"></div></div>');
		aali_tm_data_images();
...
function aali_tm_portfolio_popup(){
	...
	button.off().on('click',function(){
		...
		modalBox.addClass('opened');
		modalBox.find('.description_wrap').html(content);
		modalBox.find('.popup_details').prepend('<div class="top_image"><img src="https://shtheme.com/demosd/aali/wp-content/uploads/2022/02/4-2.jpg" alt="" /><div class="main" data-img-url="'+image+'"></div></div>');
...
function aali_tm_news_popup(){
	...
	var modalBox		= jQuery('.aali_tm_modalbox');
	var button			= jQuery('.aali_tm_news .list_inner .aali_tm_full_link,.aali_tm_news .news_list ul li .details .title a,.aali_tm_news .aali_tm_learn_more12 a');
	var closePopup		= modalBox.find('.close');
	...
	button.on('click',function(){
		...
		modalBox.addClass('opened');
		modalBox.find('.description_wrap').html(content);
		modalBox.find('.news_popup_informations').prepend('<div class="image"><img src="https://shtheme.com/demosd/aali/wp-content/uploads/2022/02/4-2.jpg" alt="" /><div class="main" data-img-url="'+image+'"></div></div>');
...
jQuery('.aali_progress').each(function() {
...
function aali_tm_page_title_margin(){
	"use strict";

	var ww				= jQuery(window).width();
	var header			= jQuery('.aali_tm_header').outerHeight(true);
	var mobMenu			= jQuery('.aali_tm_mobile_menu').outerHeight();
	var title			= jQuery('.aali_tm_page_title');
	var portfolioImage	= jQuery('.aali_tm_portfolio_single .main_image');

	if(ww >= 1040){
		title.css({marginTop:header+'px'});
		portfolioImage.css({marginTop:header+'px'});
	}else{
		title.css({marginTop:mobMenu+'px'});
		portfolioImage.css({marginTop:(mobMenu+50+'px')});
	}
...
function aali_tm_submenu(){
	
	var WW			= jQuery(window).width();
	var submenu		= jQuery('.aali_tm_header .sub-menu');
	var submenuW	= submenu.outerWidth();

	submenu.each(function(){
		var element  = jQuery(this);
		var elOffset = element.offset().left+submenuW;
		if(elOffset >= WW){
...
function aali_tm_versubmenu() {

	jQuery(".aali_tm_mobile_menu .dropdown a").on('click',function(){
		var e 	= jQuery(this);
		var p	= e.closest('li');
		var s	= p.children('.sub-menu');
		if(s.length){
			if(p.hasClass('active')){
				p.removeClass('active');
				p.find('.sub-menu').slideUp();
				p.find('.active').removeClass('active');
			}else{
				p.siblings().find('.sub-menu').slideUp();
				p.siblings().removeClass('active').find('.active').removeClass('active');
				p.addClass('active');
				s.slideDown();
			}
			return false;
